...
Единственный наблюдаемый сигнал этого в ответе Ollama — счётчик prompt_eval_count (сколько
токенов промпта реально прогнано через модель на этом вызове) и prompt_eval_duration
(сколько времени это заняло). Скрипт не воспроизводит биллинг вендоров, он показывает тот же
факт архитектуры: повторный проход по неизменному префиксу дешевле нового.
