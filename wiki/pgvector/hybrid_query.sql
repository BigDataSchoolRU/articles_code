...
-- Seq Scan для запроса с фильтром по редкой категории (25 подходящих строк
-- дешевле отсортировать перебором, чем идти в HNSW-индекс) — тогда поиск
-- точный и overfiltering не проявляется в принципе. enable_seqscan=off здесь
