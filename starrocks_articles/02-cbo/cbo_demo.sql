...
-- Ниже помечено, что именно скриншотить (компактный EXPLAIN) и какие
-- строки выносить в сравнительную таблицу (из EXPLAIN COSTS).
-- ============================================================
...
-- 2.1 СКРИНШОТ 1. Компактный план, видно порядок соединений.
EXPLAIN
...
-- 2.2 Для таблицы сравнения. Из вывода COSTS берём оценку строк (row:)
--     по узлам SCAN orders и по HASH JOIN. Без статистики оценка
--     селективности фильтра грубая, это и есть колонка "до".
EXPLAIN COSTS
SELECT c.region_id, count(*) AS cnt
FROM order_items oi
JOIN orders o    ON oi.order_id = o.order_id
...
-- 4.1 СКРИНШОТ 2. Компактный план. Сравниваем порядок соединений со Скриншотом 1.
EXPLAIN
SELECT c.region_id, count(*) AS cnt
FROM order_items oi
JOIN orders o    ON oi.order_id = o.order_id
...
-- 4.2 Колонка "после" для таблицы сравнения. Берём те же оценки row:
--     по SCAN orders и HASH JOIN. Теперь оценка селективности точная.
EXPLAIN COSTS
SELECT c.region_id, count(*) AS cnt
FROM order_items oi
JOIN orders o    ON oi.order_id = o.order_id
...
-- ------------------------------------------------------------
-- Что выносим в статью:
--   Скриншот 1: компактный EXPLAIN до ANALYZE (порядок соединений).
--   Скриншот 2: компактный EXPLAIN после ANALYZE (изменённый порядок).
--   Таблица сравнения из EXPLAIN COSTS, две колонки до и после:
--     - оценка строк SCAN orders после фильтра (row:)
--     - оценка строк на выходе первого HASH JOIN (row:)
--     - при желании итоговая cost: корневого узла
--   Полный четырёхтабличный план из статьи 1 оставляем как отсылку
--   для тех, кто хочет разбор большого запроса.
-- ------------------------------------------------------------
